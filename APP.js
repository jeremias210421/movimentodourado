...
const WhatsAppChatbotConfigurator = () => {
  ...
  const [step, setStep] = useState('test'); // Aba ativa
  const [faqList, setFaqList] = useState(defaultFAQs); // Lista de FAQ
  const [companyInfo, setCompanyInfo] = useState({ name: 'Sua Empresa' }); // Informações da empresa
  ...
  const getBotResponse = (input) => {
    ...
    const isBusinessHours = currentTime >= 9 && currentTime < 18; // Assumindo horário comercial das 9h às 18h
